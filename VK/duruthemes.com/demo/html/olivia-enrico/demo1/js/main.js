...
(function () {
    ...
    let startDate = new Date("July 30, 2022 00:00:00").getTime();

    let x = setInterval(function () {
        let now = new Date().getTime(),
            distance = now - startDate; // time passed since July 30, 2022

        if (distance < 0) {
            // Before July 30, 2022
            document.getElementById("headline").innerText = "Countdown starts on July 30, 2022!";
            document.getElementById("countdown").style.display = "none";
            document.getElementById("content").style.display = "block";
        } else {
            // Show how much time has passed since July 30, 2022
            document.getElementById("days").innerText = Math.floor(distance / (day));
            document.getElementById("hours").innerText = Math.floor((distance % (day)) / (hour));
            document.getElementById("minutes").innerText = Math.floor((distance % (hour)) / (minute));
            document.getElementById("seconds").innerText = Math.floor((distance % (minute)) / second);
            document.getElementById("headline").innerText = "Time since we met!";
